...
func TestExecute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by uuid] execute repository")
			},
			in: in{
				ctx:  ctx,
				uuid: uuid,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by uuid] execute repository")
				m.On("Error", "request timed out while get user by uuid", "err", context.DeadlineExceeded)
			},
			in: in{
				ctx:  ctx,
				uuid: uuid,
		...
		},
		{
			...
			mockLoggerBehavior: func(m *loggermocks.ILogger) {
				m.On("Debug", "[get user by uuid] execute repository")
				m.On("Error", "failed to get user by uuid", "err", errors.New("database error"))
			},
			in: in{
				ctx:  ctx,
				uuid: uuid,
			...
			want: want{
				user: user.User{},
				err:  errors.New("could not get user by uuid: database error"),
			},
		},
	}

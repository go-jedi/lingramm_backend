...
//go:generate mockery --name=IReduceUserBalance --output=mocks --case=underscore
type IReduceUserBalance interface {
	Execute(ctx context.Context, tx pgx.Tx, dto userbalance.ReduceUserBalanceDTO) (userbalance.UserBalance, error)
}

type ReduceUserBalance struct {
	queryTimeout int64
...
func (r *ReduceUserBalance) init() {
	if r.queryTimeout == 0 {
		r.queryTimeout = postgres.DefaultQueryTimeout
	}
}

// Execute performs the entire process of reduce amount to a user's balance:
...
func (r *ReduceUserBalance) Execute(ctx context.Context, tx pgx.Tx, dto userbalance.ReduceUserBalanceDTO) (userbalance.UserBalance, error) {
	r.logger.Debug("[reduce user balance] execute repository")
	...
	// current balance reduce amount.
	nb := cb.Sub(dto.Amount)
...
func (r *ReduceUserBalance) getCurrentBalance(ctx context.Context, tx pgx.Tx, telegramID string) (decimal.Decimal, error) {
...
func (r *ReduceUserBalance) createBalanceTransaction(ctx context.Context, tx pgx.Tx, dto userbalance.ReduceUserBalanceDTO, newBalance decimal.Decimal) error {
	ctxTimeout, cancel := context.WithTimeout(ctx, time.Duration(r.queryTimeout)*time.Second)
	defer cancel()
...
func (r *ReduceUserBalance) updateBalance(ctx context.Context, tx pgx.Tx, telegramID string, newBalance decimal.Decimal) (userbalance.UserBalance, error) {
	ctxTimeout, cancel := context.WithTimeout(ctx, time.Duration(r.queryTimeout)*time.Second)
	defer cancel()

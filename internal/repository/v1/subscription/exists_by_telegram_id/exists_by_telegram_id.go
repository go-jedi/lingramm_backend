...
func (r *ExistsByTelegramID) Execute(ctx context.Context, tx pgx.Tx, telegramID string) (bool, error) {
	r.logger.Debug("[check subscription exists by telegram id] execute repository")
	...
	if err := tx.QueryRow(
		...
		if errors.Is(err, context.DeadlineExceeded) {
			r.logger.Error("request timed out while check subscription exists by telegram id", "err", err)
			return false, fmt.Errorf("the request timed out: %w", err)
		}
		r.logger.Error("failed to check subscription exists by telegram id", "err", err)
		return false, fmt.Errorf("could not check subscription exists by telegram id: %w", err)
	}

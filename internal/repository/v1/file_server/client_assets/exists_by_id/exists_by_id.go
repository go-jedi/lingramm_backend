...
func (r *ExistsByID) Execute(ctx context.Context, tx pgx.Tx, id int64) (bool, error) {
	r.logger.Debug("[check client assets exists by id] execute repository")
	...
	if err := tx.QueryRow(
		...
		if errors.Is(err, context.DeadlineExceeded) {
			r.logger.Error("request timed out while check client assets exists by id", "err", err)
			return false, fmt.Errorf("the request timed out: %w", err)
		}
		r.logger.Error("failed to check client assets exists by id", "err", err)
		return false, fmt.Errorf("could not check client assets exists by id: %w", err)
	}

...
func (r *All) Execute(ctx context.Context, tx pgx.Tx) ([]clientassets.ClientAssets, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&clientAsset.ID, &clientAsset.NameFile, &clientAsset.NameFileWithoutExtension,
			&clientAsset.ServerPathFile, &clientAsset.ClientPathFile, &clientAsset.Extension,
			&clientAsset.Quality, &clientAsset.OldNameFile, &clientAsset.OldExtension,

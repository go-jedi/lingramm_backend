...
func (r *ExistsByName) Execute(ctx context.Context, tx pgx.Tx, name string) (bool, error) {
	r.logger.Debug("[check event type exists by name] execute repository")
	...
	if err := tx.QueryRow(
		...
		if errors.Is(err, context.DeadlineExceeded) {
			r.logger.Error("request timed out while check event type exists by name", "err", err)
			return false, fmt.Errorf("the request timed out: %w", err)
		}
		r.logger.Error("failed to check event type exists by name", "err", err)
		return false, fmt.Errorf("could not check event type exists by name: %w", err)
	}

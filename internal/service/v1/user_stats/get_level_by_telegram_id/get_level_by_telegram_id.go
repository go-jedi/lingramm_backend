...
type IGetLevelByTelegramID interface {
	Execute(ctx context.Context, telegramID string) (userstats.GetLevelByTelegramIDResponse, error)
}
...
func (s *GetLevelByTelegramID) Execute(ctx context.Context, telegramID string) (userstats.GetLevelByTelegramIDResponse, error) {
	s.logger.Debug("[get level user by telegram id] execute service")

	var (
		err             error
		result          userstats.GetLevelByTelegramIDResponse
		userExists      bool
	...
	if err != nil {
		return userstats.GetLevelByTelegramIDResponse{}, err
	}
	defer func() {
		if err != nil {
	...
	// get user level by telegram id.
	result, err = s.userStatsRepository.GetLevelByTelegramID.Execute(ctx, tx, telegramID)
	if err != nil {
		return userstats.GetLevelByTelegramIDResponse{}, err
	}

...
type IUpdate interface {
	Execute(ctx context.Context, dto userstudiedlanguage.UpdateDTO) (userstudiedlanguage.UserStudiedLanguage, error)
}
...
func (s *Update) Execute(ctx context.Context, dto userstudiedlanguage.UpdateDTO) (userstudiedlanguage.UserStudiedLanguage, error) {
	s.logger.Debug("[update user studied language] execute service")
	...
	if !userStudiedExists { // if user studied language does not exist.
		err = apperrors.ErrUserStudiedLanguageDoesNotExist
		return userstudiedlanguage.UserStudiedLanguage{}, err
	}

	// update user studied language.
	result, err = s.userStudiedLanguageRepository.Update.Execute(ctx, tx, dto)
	if err != nil {
		return userstudiedlanguage.UserStudiedLanguage{}, err
	}

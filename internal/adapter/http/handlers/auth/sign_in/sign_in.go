...
func (si *SignIn) Execute(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&dto); err != nil {
		...
		return c.JSON(response.New(false, "failed to bind body", err.Error(), nil))
	}

	if err := si.validator.Struct(dto); err != nil {
		si.logger.Error("failed to validate struct", "error", err)
		c.Status(fiber.StatusBadRequest)
		return c.JSON(response.New(false, "failed to validate struct", err.Error(), nil))
	}
	...
	return c.JSON(response.New(true, "success", "", result))
}

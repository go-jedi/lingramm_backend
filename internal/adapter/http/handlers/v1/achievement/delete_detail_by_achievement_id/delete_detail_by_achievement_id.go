...
func (h *DeleteDetailByAchievementID) Execute(c fiber.Ctx) error {
	...
	result, err := h.achievementService.DeleteDetailByAchievementID.Execute(ctxTimeout, achievementID)
	if err != nil {
		h.logger.Error("failed to delete detail by achievement id", "error", err)
		c.Status(fiber.StatusInternalServerError)
		return c.JSON(response.New[any](false, "failed to delete detail by achievement id", err.Error(), nil))
	}
